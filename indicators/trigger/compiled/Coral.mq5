...
input double InpCoeff = 0.063492063492;
...
int OnInit(){
    ...
    for(int i = 0; i < ArraySize(SymbolsArray); i++){
        indicator_handle[i] = iCustom(SymbolsArray[i], PERIOD_CURRENT, "MyIndicators/Coral.ex5", InpCoeff, InpAppliedPrice);
    }
...
void trigger(int indi_handle, bool &trigger_long, bool &trigger_short){
    ...
    CopyBuffer(indi_handle, 1, 1, 10, BufferColors);

    trigger_long  = (BufferColors[0] == 0 && BufferColors[1] == 1);
    trigger_short = (BufferColors[0] == 1 && BufferColors[1] == 0);
}