...
from strategy_factory.renderer_tools import build_input_lines, load_results_data
...
def render_exit(project_config: ProjectConfig, stage_config: StageConfig, indi_name: str, indi_data: dict) -> str:
    """Render function for the exit stage.
    ...
    - indi_name: Name of the exit indicator.
    ...
    exit_logic_inputs = indi_data.get("logic_inputs") or {}
    exit_logic_inputs_vars = list(exit_logic_inputs.keys())
    exit_logic_inputs_defaults = [exit_logic_inputs[k]["default"] for k in exit_logic_inputs_vars]

    # Volume stage
    volume_stage = get_stage_config(stages, "Volume")
    volume_name, volume_data = load_results_data(project_config.run_name, volume_stage)
    volume_logic_inputs = volume_data.get("logic_inputs", {})
    ...
    rendered_ea = stage_config.ea_template.render(
        ...
        # Exit indicator
        exit_logic_inputs_vars=exit_logic_inputs_vars,
        exit_logic_inputs_defaults=exit_logic_inputs_defaults,
        exit_custom=indi_data.get("custom"),
        exit_function=indi_data.get("function"),
        exit_indicator_path=indi_data.get("indicator_path"),
        exit_inputs_vars=[v["default"] for v in indi_data["indicator_inputs"].values()],
        exit_buffers=indi_data.get("buffers", []),
        exit_long_conditions=indi_data.get("trigger_conditions", {}).get("short"),  # Exits are inverted triggers
        exit_short_conditions=indi_data.get("trigger_conditions", {}).get("long"),  # Exits are inverted triggers

        # Volume indicator
        volume_logic_inputs_vars=volume_logic_inputs_vars,
        volume_logic_inputs_defaults=volume_logic_inputs_defaults,
        volume_custom=volume_data.get("custom"),
        volume_function=volume_data.get("function"),
        volume_indicator_path=volume_data.get("indicator_path"),
        volume_inputs_vars=[v["default"] for v in volume_data["indicator_inputs"].values()],
        volume_buffers=volume_data.get("buffers", []),
        volume_long_conditions=volume_data.get("volume_conditions", {}).get("long"),
        volume_short_conditions=volume_data.get("volume_conditions", {}).get("short"),
