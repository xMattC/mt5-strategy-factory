...
int OnInit() {
    ...
    for(int i = 0; i < ArraySize(SymbolsArray); i++) {
        trig_handle[i] = iADX(SymbolsArray[i], PERIOD_CURRENT, 37);
        conf_handle[i] = iRSI(SymbolsArray[i], PERIOD_CURRENT, 14, PRICE_CLOSE);
        tl_handle[i] = iMA(SymbolsArray[i], PERIOD_CURRENT, InpPeriod, InpShift, InpMAType, InpAppliedPrice);
        //---
...
struct trading_signals {
    bool trigger_sig; bool last_trigger_sig; bool trigger_agrees;   // Trigger Signals
    bool conformation;                                              // Conformation Signals
    bool tl_cross; bool trend_direction;                            // Tendline Signals
    bool inside_atr_band;                                           // ATR Bands Signals
    bool tl_cross_exit;                                             // Exit Signals
};
...
trading_signals build_trading_signals(bool is_long, string symbol, int tl_hand, int trig_hand, int conf_hand) {
    ...
    // Trigger Signals
    trigger(trig_hand, long_signals.trigger_sig, short_sigs.trigger_sig, long_signals.trigger_agrees, short_sigs.trigger_agrees);
    track_trigger_signal.get_tracker(symbol).update_signal_tracker(long_signals.trigger_sig, short_sigs.trigger_sig);
    ...
    // Trendline Signals
    int tl_buffer = 0;
    tl_tools.detect_cross(symbol, tl_hand, 0, long_signals.tl_cross, short_sigs.tl_cross, 1);
    tl_tools.trend_direction(symbol, tl_hand, 0, long_signals.trend_direction, short_sigs.trend_direction, 1);
    ...
    // Exit Signals
    long_signals.tl_cross_exit = short_sigs.tl_cross;  // Long exits when price crosses below the trendline
    short_sigs.tl_cross_exit = long_signals.tl_cross;  // Short exits when price crosses above the trendline
...
bool standard_entry(const trading_signals& data) {
    ...
    return trigger_sig && trend_direction && inside_atr_band && confirmation;
}
...
bool trendline_cross_entry(const trading_signals& data) {
    ...
    return tl_cross && trigger_agrees && trigger_sig_recent && inside_atr_band && confirmation;
}
...
void conformation(int handle, bool& long_ok, bool& short_ok) {
    ...
    CopyBuffer(handle, 0, 1, 10, RSI);
    
    long_ok = (RSI[0] > rsi_mid_threshold);
    short_ok = (RSI[0] < rsi_mid_threshold);
}