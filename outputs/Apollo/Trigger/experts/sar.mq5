...
//+------------------------------------------------------------------+
//--- Indicator inputs
input double InpStep = 0.02;
input double InpMaximum = 0.2;
//--- Logic inputs
//+------------------------------------------------------------------+
...
int OnInit(){
    ...
    for(int i = 0; i < ArraySize(SymbolsArray); i++) {
        trig_handle[i] = iSAR(SymbolsArray[i], PERIOD_CURRENT, InpStep, InpMaximum);
...
void trigger(int handle, bool& long_sig, bool& short_sig) {
    ...
    long_sig  = (close[0] > SAR[0] && close[1] < SAR[1]);
    short_sig = (close[0] < SAR[0] && close[1] > SAR[1]);
}