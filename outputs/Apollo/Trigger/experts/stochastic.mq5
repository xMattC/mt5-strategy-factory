...
int OnInit(){
    ...
    for(int i = 0; i < ArraySize(SymbolsArray); i++) {
        trig_handle[i] = iStochastic(SymbolsArray[i], PERIOD_CURRENT, InpKPeriod, InpDPeriod, InpSlowing, InpMAType, InpAppliedPrice);
...
void trigger(int handle, bool& long_sig, bool& short_sig) {
    ...
    CopyBuffer(handle, 0, 1, 10, K);
    
    double D[];
    ArraySetAsSeries(D, true);
    CopyBuffer(handle, 1, 1, 10, D);
    
    long_sig  = (K[0] > D[0] && K[1] < D[1]);
    short_sig = (K[0] < D[0] && K[1] > D[1]);
}