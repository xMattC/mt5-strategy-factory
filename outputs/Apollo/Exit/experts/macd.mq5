...
int OnInit() {
    ...
    for(int i = 0; i < ArraySize(SymbolsArray); i++) {
        ...
        exit_handle[i] = iMACD(SymbolsArray[i], PERIOD_CURRENT, 12, 26, 9, PRICE_CLOSE);
        //---
...
void exit_signal(int handle, bool& exit_long, bool& exit_short) {
    ...
    exit_long = (MACD[0] < Signal[0] && MACD[1] > Signal[1]);
    exit_short = (MACD[0] > Signal[0] && MACD[1] < Signal[1]);
}